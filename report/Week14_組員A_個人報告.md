...
**課程**：LLM Applications in Cybersecurity — Proof of Concept Development
**繳交週次**：Week 14（環境建置與模組原型）
...
> [📷 截圖 1：補上一張用 draw.io / Excalidraw 畫的彩色架構圖，會比 ASCII 漂亮很多]
...
> [📷 截圖 2：`chrome://extensions/` 載入未封裝項目後，擴充功能成功顯示的截圖]
...
> [📷 截圖 8：Options 頁的「自訂掃描網站」UI，包含新增表單與已註冊網站清單]

> [📷 截圖 9：點「＋ 新增」後 Chrome 跳出的原生授權彈窗（「允許 / 拒絕」對話框）]

#### 在期末報告的論述價值

這段設計可在 Week 16 報告呼應課程強調的 **Principle of Least Privilege (PoLP)** 與 **Just-in-Time Authorization**：
...
> [📷 截圖 4：瀏覽器開啟 `http://127.0.0.1:8080/docs` 看到 Swagger UI 自動生成的 API 文件]

> [📷 截圖 5：在 Swagger UI 內輸入測試文本後，後端回傳的 JSON 結果（顯示 High / Urgency+Authority）]
...
> [📷 截圖 6：T1 測試信件在 Gmail 上方成功跳出紅色警告 Overlay 的畫面]

> [📷 截圖 7：T3 正常信件未觸發警告（驗證沒有 False Positive）]
...
本週與組員 B、C 各開了一次同步會議，確認以下契約：

- **輸入文本格式**：純文字字串，不含 HTML，最長 20,000 字（前端 5,000 字 + 後端 20,000 字雙層防護）
- **輸出 JSON Schema**：見 §5.1，所有欄位名稱凍結，組員 C 的 Prompt 必須產出符合該結構的輸出
- **意圖分類四種**：Urgency、Authority、Financial、Greed（由組員 C 提出資安學理依據）
- **risk_level 三級**：High / Medium / Low（前端對應紅 / 橘 / 藍配色）

> [📷 截圖 10：可附上組員會議記錄或 Notion 截圖，證明三人有達成 schema 共識]
